...
This directory holds coordination assets for running multiple Claude Code agents in parallel while they work on tasks in the Rack Gateway repository.
...
3. **Launch Claude Code** inside the worktree and feed it the relevant prompt template:
   ```bash
   claude code --prompt-file ai_automation/agents/worker_prompt_template.txt
   ```
...
4. **Register the worker PID** so the supervisor scripts can track completion:
   ```bash
   # in another terminal
   ai_automation/agents/register_worker.sh 001
   # or provide the PID explicitly if multiple agents are running
   ai_automation/agents/register_worker.sh 001 12345
   ```
5. The agent follows the project rules in `AGENTS.md`, fixes the task, runs the required `task` commands, and opens a PR-ready branch.
6. To monitor workers automatically, run `ai_automation/agents/wait_for_workers.sh` from the main repo. It watches the registered PIDs and returns as soon as one task completes (or times out after 20 minutes).

### Shared Postgres Container

- `ai_automation/agents/run_worker.sh` exports `RGW_SHARED_DB_PROJECT` so every worktree shares the primary `rack-gateway-postgres-1` container from the main checkout.
...
- Capture any cross-task constraints or shared utilities in commit messages or `ai_automation/duplication_tasks.md`.
