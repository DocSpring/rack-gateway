...
function createAuditQueryParams(
  options: AuditQueryParamsOptions,
): URLSearchParams {
...
function appendPagination(
  ...
  const shouldIncludePage =
    typeof page === 'number' &&
    page > 0 &&
    (includeDefaultPagination || page > 1);
...
export function AuditPage({
  ...
  const [resourceTypeFilter, setResourceTypeFilter] = useState(() =>
    ensureFilterValue(
      initialSearchParams?.get('resource_type'),
      RESOURCE_TYPES,
    ),
  );
  ...
  const [queryUserId] = useState(
    () => initialSearchParams?.get('user_id') ?? '',
  );
  ...
  const deniedEvents = countEvents(
    (l) => l.status === 'denied' || l.status === 'blocked',
  );
  ...
  const pageStats = {
    ...
    denied: deniedEvents,
    avgResponseTime:
      totalEvents > 0 ? Math.round(totalResponseTime / totalEvents) : 0,
  };
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl text-green-600">
              {pageStats.total > 0
                ? Math.round((pageStats.success / pageStats.total) * 100)
                : 0}
              %
            </div>
          </CardContent>
...
function DateTimePickerField({
  ...
  const timeMin =
    minValue && minParts.datePart === datePart ? minParts.timePart : undefined;
  const timeMax =
    maxValue && maxParts.datePart === datePart ? maxParts.timePart : undefined;
