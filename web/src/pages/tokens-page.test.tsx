...
async function createStepUpMock() {
  ...
  const MockStepUpProvider = ({ children }: { children: ReactNode }) => (
    <StepUpContext.Provider value={stepUpContextStub}>
      {children}
...
describe('TokensPage', () => {
  ...
  describe('Token List', () => {
    ...
    it('shows empty state when no tokens', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('No API tokens created yet'),
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Token Creation', () => {
    it('opens create token dialog', async () => {
      ...
      expect(
        screen.getByRole('checkbox', { name: APP_LIST_REGEX }),
      ).toBeChecked();
      expect(
        screen.getByRole('checkbox', { name: WILDCARD_REGEX }),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(ALL_HEADING_REGEX, { selector: 'p' }),
    ...
    it('creates a new token and displays it', async () => {
      ...
      await waitFor(() => {
        expect(api.post).toHaveBeenCalledWith('/api/v1/admin/tokens', {
          name: 'New Token',
          permissions: defaultPermissions,
        });
      });
    ...
    it('copies token to clipboard', async () => {
      ...
      const tokenSecretSection = screen
        .getByText('Token Secret')
        .closest('div');
    ...
    it('applies role shortcut selections', async () => {
      ...
      expect(
        screen.getByRole('checkbox', { name: RELEASE_PROMOTE_REGEX }),
      ).not.toBeChecked();
    ...
    it('disables permissions that exceed current user roles', async () => {
      const deployerMetadata = {
        ...
        permissions: [
          ...mockPermissionMetadata.permissions,
          'convox:rack:update',
        ],
        user_roles: ['deployer'],
      ...
      vi.mocked(api.get)
        .mockResolvedValueOnce(deployerMetadata)
        .mockResolvedValueOnce(mockTokens);
  ...
  describe('Token Deletion', () => {
    it('deletes a token', async () => {
      ...
      await waitFor(() => {
        expect(api.delete).toHaveBeenCalledWith('/api/v1/admin/tokens/tok-1');
      });
    });
  });
  ...
  describe('Error Handling', () => {
    it('displays error when loading fails', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Failed to load API tokens'),
        ).toBeInTheDocument();
      });
    });
      ...
      getMock.mockImplementation((url: string) => {
        if (url.includes('/tokens/permissions')) {
          return Promise.resolve(
            mockPermissionMetadata as unknown as APIToken[],
          );
        }
