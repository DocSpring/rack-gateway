...
const CLI_ERROR_MESSAGES: Record<string, string> = {
  ...
  state_and_code_required:
    'State and code are both required to approve the login.',
  load_failure:
    'We could not load the login session. Try again from your terminal.',
  exchange_failed:
  ...
  service_unavailable:
    'Login approval is temporarily unavailable. Try again shortly.',
...
export function MFAChallengePage() {
  ...
  const channel =
    extractParam(search, 'channel') ?? extractParam(search, 'flow');
  ...
  >({
    ...
    onSuccess: (result) => {
      if (mode === 'cli') {
        const target = result?.redirect?.trim();
        const destination =
          target && target !== '' ? target : WebRoute('cli/auth/success');
            ...
            onMFAStatusLoaded={(mfaStatus) => {
              ...
              if (
                ...
                window.location.assign(
                  `${WebRoute('account/security')}?${params.toString()}`,
                );
              }
            }}
