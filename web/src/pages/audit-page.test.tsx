...
describe('AuditPage', () => {
    ...
    it('renders API token metadata in detail dialog', async () => {
      ...
      expect(
        within(dialog).getByText((_, element) => {
          ...
          return (
            element.textContent === 'Owner: cibot@example.com (CI Bot Owner)'
          );
        }),
      ).toBeInTheDocument();
    });
  ...
  describe('Statistics', () => {
    it('calculates and displays statistics correctly', async () => {
      ...
      await waitFor(() => {
        const totalLogsCard =
          screen.getByText('Total Logs').parentElement?.parentElement;
        if (!totalLogsCard) {
        ...
        const successRateCard =
          screen.getByText('Success Rate').parentElement?.parentElement;
        if (!successRateCard) {
        ...
        const failedCard =
          screen.getByText('Failed/Denied').parentElement?.parentElement;
        if (!failedCard) {
        ...
        const responseCard =
          screen.getByText('Avg Response Time').parentElement?.parentElement;
        if (!responseCard) {
  ...
  describe('Filtering', () => {
    it('filters by action type', async () => {
      ...
      vi.mocked(api.get)
        ...
        .mockResolvedValueOnce(
          makeResponse(mockLogs.filter((l) => l.action_type === 'auth')),
        );
    ...
    it('filters by resource type', async () => {
      ...
      vi.mocked(api.get)
        .mockResolvedValueOnce(makeResponse(mockLogs))
        .mockResolvedValueOnce(
          makeResponse(mockLogs.filter((l) => l.resource_type === 'user')),
        );
      ...
      await waitFor(() => {
        expect(api.get).toHaveBeenCalledWith(
          expect.stringContaining('resource_type=api_token'),
        );
      });
    });
    ...
    it('filters by status', async () => {
      ...
      vi.mocked(api.get)
        .mockResolvedValueOnce(makeResponse(mockLogs))
        .mockResolvedValueOnce(
          makeResponse(mockLogs.filter((l) => l.status === 'failed')),
        );
    ...
    it('searches by text', async () => {
      ...
      vi.mocked(api.get)
        .mockResolvedValueOnce(makeResponse(mockLogs))
        .mockResolvedValueOnce(
          makeResponse(mockLogs.filter((l) => l.user_email.includes('admin'))),
        );
  ...
  describe('URL synchronization', () => {
    ...
    it('clamps the page to the available total when the response has fewer pages', async () => {
      ...
      await waitFor(
        () => {
          expect(api.get).toHaveBeenCalledWith(
            expect.stringMatching(PAGE_FIVE_REGEX),
          );
        },
        { timeout: 2000 },
      );
      ...
      await waitFor(
        () => {
          expect(api.get).toHaveBeenCalledWith(
            expect.stringMatching(PAGE_TWO_REGEX),
          );
        },
        { timeout: 2000 },
      );
