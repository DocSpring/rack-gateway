...
import {
  buildCircleCIPipelineUrl,
  extractCircleCIMetadata,
} from '@/lib/ci-utils';
...
export function DeployApprovalRequestDetailPage() {
  ...
  const approveMutation = useMutation({
    mutationFn: (requestId: string) =>
      approveDeployApprovalRequest(requestId, {}),
    onSuccess: () => {
  ...
  const approveRequest = useCallback(
    async (requestId: string) => {
      try {
        ...
        if (
          handleStepUpError(err, () => approveMutation.mutateAsync(requestId))
        ) {
          return;
        }
  ...
  const {
    ...
    queryKey: [
      'deployApprovalRequestAuditLogs',
      id,
      auditPageIndex,
      DEFAULT_PER_PAGE,
    ],
    queryFn: () =>
      api.get(
        `/api/v1/admin/deploy-approval-requests/${id}/audit-logs?limit=${DEFAULT_PER_PAGE}`,
      ),
    enabled: !!request,
    ...
    return (
      <div className="space-y-4 p-8">
        ...
        <p className="text-destructive text-sm">
          Unable to load request: {requestError.message}
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            {request?.status && (
              <DeployApprovalStatusBadge status={request.status} />
            )}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent className="space-y-3">
            ...
            {circleCIMetadata?.pipelineNumber && (
              <DetailRow
                ...
                value={circleCIMetadata.pipelineNumber}
              />
            )}
            {circleCIPipelineUrl && (
              <DetailRow
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <DetailRow
              label="Updated At"
              value={renderTime(request?.updated_at)}
            />
            <DetailRow
              label="Expires At"
              value={renderTime(request?.approval_expires_at)}
            />
            <DetailRow
              label="Approved By"
              value={formatUser(
            ...
            <DetailRow
              label="Approved At"
              value={renderTime(request?.approved_at)}
            />
            ...
            {request?.rejected_at && (
              <DetailRow
                label="Rejected At"
                value={renderTime(request.rejected_at)}
              />
            )}
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(
                    request.exec_commands as Record<string, string>,
                  ).map(([processId, command]) => (
