...
export function IntegrationsPage() {
  ...
  const { data: circleCISettings } = useQuery<CircleCISettings | null>({
    ...
    queryFn: async (): Promise<CircleCISettings | null> => {
      try {
        const response = await api.get<CircleCISettings>(
          '/api/v1/admin/settings/circleci',
        );
        return response;
      } catch (error: unknown) {
  ...
  const { data: gitHubSettings } = useQuery<GitHubSettings | null>({
    ...
    queryFn: async (): Promise<GitHubSettings | null> => {
      try {
        const response = await api.get<GitHubSettings>(
          '/api/v1/admin/settings/github',
        );
        return response;
      } catch (error: unknown) {
        if (hasStatus(error, 404)) {
  ...
  const { data: integration, isLoading } = useQuery<SlackIntegration | null>({
    ...
    queryFn: async (): Promise<SlackIntegration | null> => {
      try {
        const response = await api.get<SlackIntegration>(
  ...
  const testMutation = useMutation({
    mutationFn: async (channelId: string) => {
      await api.post('/api/v1/admin/integrations/slack/test', {
  ...
  const handleRemoveAction = (key: string, actionIndex: number) => {
    ...
    const updatedActions = {
      ...
      [key]: {
        ...currentConfig,
        actions: currentConfig.actions.filter(
          (_: string, i: number) => i !== actionIndex,
        ),
      },
    };
    ...
    <div className="container mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Connect external services to receive notifications and automate
          workflows
        </p>
      </div>
              ...
              {circleCIEnabled ? (
                <div className="space-y-3">
                  <Alert>
                    ...
                    <AlertDescription>
                      CircleCI integration is enabled. The gateway will
                      automatically approve CircleCI jobs when deploy approvals
                      are granted.
                    </AlertDescription>
                  </Alert>
                  ...
                  <p className="text-muted-foreground text-xs">
                    Per-app organization slugs and job names are configured in
                    app settings.
                  </p>
                </div>
              ) : (
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <CardDescription>
                    Verify commits and PRs for deployments
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  {gitHubEnabled ? (
              ...
              {gitHubEnabled ? (
                <div className="space-y-3">
                  <Alert>
                    <CheckCircle2 className="size-4" />
                    <AlertDescription>
                      GitHub integration is enabled. The gateway can verify git
                      commits and PRs for deploy approval requests.
                    </AlertDescription>
                  </Alert>
                  <div className="rounded border bg-muted p-4">
                    <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-xs">
                    ...
                    env var. Per-app repositories are configured in app
                    settings.
                  </p>
                </div>
              ) : (
              ...
              {integration ? (
                <div className="space-y-6">
                  <Alert>
                    <CheckCircle2 className="size-4" />
                    <AlertDescription>
                      Slack is connected. Configure which channels receive
                      notifications below.
                    </AlertDescription>
                  </Alert>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {Object.entries(integration.channel_actions).map(
                        ([key, config]) => (
                          <ChannelConfigCard
                ...
                <div className="flex flex-col gap-4 pb-4">
                  <p className="mb-4 text-muted-foreground text-sm">
                    Connect your Slack workspace to receive notifications for
                    security events and deploy approvals.
                  </p>
                  <div>
                    <Button
...
function ChannelConfigCard({
  ...
  const handleActionsChange = (newActions: string[]) => {
    ...
    if (newActions.length > config.actions.length) {
      ...
      const removedIndex = config.actions.findIndex(
        (a) => !newActions.includes(a),
      );
      if (removedIndex !== -1) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-muted-foreground text-xs">
            Use glob patterns like <code>mfa.*</code> to match actions. View
            audit logs to see available actions.
          </p>
        </div>
      </CardContent>
    </Card>
