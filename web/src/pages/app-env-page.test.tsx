...
describe('AppEnvPage', () => {
  ...
  it('allows editors to update environment values and saves changes', async () => {
    ...
    await waitFor(() =>
      expect(mockUpdateAppEnv).toHaveBeenCalledWith(
        'myapp',
        { FOO: 'baz' },
        [],
      ),
  ...
  it('sends removed keys when deleting variables', async () => {
    ...
    await waitFor(() =>
      expect(mockUpdateAppEnv).toHaveBeenCalledWith('myapp', {}, ['FOO']),
    );
  });
  ...
  it('reveals secret values on demand and hides again', async () => {
    ...
    const initialValueInputs =
      await screen.findAllByLabelText('Environment value');
    ...
    await waitFor(() =>
      expect(mockFetchAppEnvValue).toHaveBeenCalledWith(
        'myapp',
        'SECRET_KEY',
        true,
      ),
    ...
    await waitFor(() =>
      expect(screen.getByDisplayValue('shhh')).toBeInTheDocument(),
    );
    await waitFor(() => {
  ...
  it('shows an error when masked sentinel is used for a new secret', async () => {
    ...
    const maskedError = new Error(
      'Masked secret value submitted without an existing secret.',
    );
    ...
    await waitFor(() =>
      expect(mockToast.error).toHaveBeenCalledWith(maskedError.message),
    );
  });
