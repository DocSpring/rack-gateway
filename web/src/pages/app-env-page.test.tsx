...
describe('AppEnvPage', () => {
  ...
  it('allows editors to update environment values and saves changes', async () => {
    ...
    mockUpdateAppEnv.mockResolvedValue({ env: { FOO: 'baz', SECRET_KEY: MASK } })
  ...
  it('reveals secret values on demand and hides again', async () => {
    ...
    const revealButtons = await screen.findAllByRole('button', { name: /Reveal secret/i })
  ...
  it('syncs with backend after save completes', async () => {
    ...
    mockUpdateAppEnv.mockResolvedValue({ env: { FOO: 'baz', SECRET_KEY: MASK }, release_id: 'R2' })
