...
export function useAuditMetrics({
	...
	return useMemo(() => calculateAuditMetrics({ logs, perPage, totalCount, page }), [
		logs,
		page,
		perPage,
		totalCount,
...
export function calculateAuditMetrics({
	...
	const firstRowIndex =
		normalizedTotalCount === 0 ? 0 : (currentPage - 1) * normalizedPerPage + 1
	const lastRowIndex =
		normalizedTotalCount === 0 ? 0 : firstRowIndex + logs.length - 1
