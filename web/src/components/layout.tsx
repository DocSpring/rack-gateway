...
function isNavigationItemActive(
  ...
  if (item.href === '/users') {
    return (
      pathname === '/users' ||
      (pathname.startsWith('/users/') && !USER_AUDIT_RE.test(pathname))
    );
  }
  if (item.href === '/') {
    return pathname === '/';
...
export function Layout() {
  ...
  if (pathname === '/') {
    return (
      <Navigate
        replace
        to={needsMfaEnrollment ? '/account/security' : '/rack'}
      />
    );
  }
          ...
          {user?.rack && (
            <div className="mb-3 text-muted-foreground text-xs">
              <div className="group relative inline-flex items-center">
                <span>
                  Rack: {user.rack.alias || user.rack.name || 'Unknown'}
                </span>
                <div
        ...
        {needsMfaEnrollment ? (
          <div className="bg-destructive text-destructive-foreground">
            <div className="px-6 py-3 font-semibold text-sm">
              Multi-factor authentication is required before you can access the
              rest of the app. Visit{' '}
              <span className="underline">Account Security</span> to finish
              setup.
            </div>
          </div>
        ) : null}
