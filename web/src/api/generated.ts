...
export const getRackGatewayAPI = () => {
  ...
  /**
   * Returns a list of deploy approval requests with optional filtering
  ...
  /**
   * Approves a deploy approval request and optionally triggers CircleCI job approval
  ...
  /**
   * Rejects a deploy approval request with optional notes
   ...
   */
  const getIntegrationsSlack = (
    options?: SecondParameter<typeof createGatewayClient<DbSlackIntegration>>,
  ) => {
    return createGatewayClient<DbSlackIntegration>(
      { url: `/integrations/slack`, method: 'GET' },
      options,
    );
  };
...
>;
export type GetIntegrationsSlackResult = NonNullable<
  Awaited<
