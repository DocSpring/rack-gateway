...
func (h *AdminHandler) ListUserSessions(c *gin.Context) {
	...
	result := h.convertSessionsToResponse(sessions)
...
func (h *AdminHandler) RevokeUserSession(c *gin.Context) {
	...
	sessionID, ok := h.parseSessionID(c, email, start)
	if !ok {
		return
	}

	_, _, ok = h.validateSessionForUser(c, email, sessionID, start)
	if !ok {
		return
	}
...
func (h *AdminHandler) parseSessionID(c *gin.Context, email string, start time.Time) (int64, bool) {
...
func (h *AdminHandler) validateSessionForUser(
	c *gin.Context,
	email string,
	sessionID int64,
	start time.Time,
) (*db.User, *db.UserSession, bool) {
	user, err := h.database.GetUser(email)
	if err != nil {
		h.respondAuditError(
			c,
