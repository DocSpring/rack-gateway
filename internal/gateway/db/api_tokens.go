...
func (d *Database) GetAPITokenByPublicID(publicID string) (*APIToken, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to query api token by public id: %w", err)
	}
	return d.GetAPITokenByID(id)
}
...
// HasActiveDeployApprovalForApp checks if an API token has an active deploy approval for a specific app
func (d *Database) HasActiveDeployApprovalForApp(tokenID int64, app string) (bool, error) {
	var count int
	err := d.queryRow(`
		SELECT COUNT(*)
