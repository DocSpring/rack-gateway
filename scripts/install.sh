...
echo "  Local dev example (if your gateway runs on localhost):"
echo "    # interactive (opens browser; CLI finishes automatically)"
echo "    convox-gateway login local http://localhost:${GW_PORT}"
echo ""
echo "    # headless / automation (same flow; no browser open)"
echo "    # writes AUTH_URL/STATE/CODE_VERIFIER to a file while CLI waits and polls"
echo "    AUTH_FILE=/tmp/convox-gateway-auth.env"
echo "    convox-gateway login local http://localhost:${GW_PORT} --no-open --auth-file \"$AUTH_FILE\" &"
echo "    # open the printed AUTH_URL in a browser (or curl it to follow redirects)"
echo "    source \"$AUTH_FILE\" && echo \"Open: $AUTH_URL\""
echo "    wait # CLI completes once the gateway callback is hit"
