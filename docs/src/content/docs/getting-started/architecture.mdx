...
│                      │                                           │
│                      ▼                                           │
│           RACK_URL=https://convox:<token>@gateway                │
└──────────────────────┼───────────────────────────────────────────┘
                       │
                       │ HTTPS
                       ▼
┌──────────────────────────────────────────────────────────────────┐
│                     Gateway Server (per rack)                     │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │
│  │    Auth     │  │    RBAC     │  │   Proxy     │              │
│  │  (OAuth)    │──│  (Roles)    │──│ (Forward)   │──────────────┼──▶ Convox Rack
│  └─────────────┘  └─────────────┘  └─────────────┘              │     (real API)
│         │                                 │                      │
│         ▼                                 ▼                      │
│  ┌─────────────┐                  ┌─────────────┐               │
...
│  ┌─────────────────────────────────────────────────┐            │
│  │              PostgreSQL Database                 │            │
│  │  (users, sessions, tokens, audit_logs, etc.)    │            │
│  └─────────────────────────────────────────────────┘            │
└──────────────────────────────────────────────────────────────────┘
```
...
1. CLI sets `RACK_URL=https://convox:<session-token>@gateway.example.com`
...
The `rack-gateway` CLI is required for interacting with Rack Gateway. It provides:
...
<Aside type="note" title="Required CLI">
Users must install and use the `rack-gateway` CLI to interact with the gateway. It handles authentication and wraps convox commands transparently.
</Aside>
...
├── rbac_decision, latency_ms
└── (request_body, response_body - redacted)
```
...
By default, Convox exposes its API publicly via nginx ingress. There's an [open PR to add a `private_api` parameter](https://github.com/convox/convox/pull/935) that disables public access entirely, restricting the API to VPC/private network access only.

DocSpring maintains a [public fork of Convox](https://github.com/DocSpring/convox/tree/docspring) that includes this feature (along with CloudFlare DNS-01 resolver support and other improvements). You can use it in your Terraform configuration:
...
module "system" {
  ...
  # Private Convox API access
  private_api = true
}
```

When `private_api = true`, the public nginx ingress is not created. You then provide access through Tailscale, VPC peering, or another private network solution.

<Aside type="note" title="Why a Fork?">
The `private_api` feature is unlikely to be merged upstream since it would prevent Convox's hosted Console from connecting to racks. If you're using Rack Gateway instead of Console, this isn't a concern.
</Aside>
