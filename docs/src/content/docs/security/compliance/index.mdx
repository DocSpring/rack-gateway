...
| Data retention | Configurable policies | Retention settings |
| Data deletion | Automatic after retention | Cleanup jobs |
...
Common queries for compliance evidence:

```bash
# All admin actions in a time period
rack-gateway audit search --role admin --from "2024-01-01" --to "2024-03-31"

# Failed access attempts
rack-gateway audit search --decision deny --days 90

# User activity for access review
rack-gateway audit search --user alice@example.com --days 90

# Deployment activity
rack-gateway audit search --action deploy --days 30
```
