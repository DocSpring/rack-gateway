...
| C1.2 | Dispose of confidential data | Session expiration + token revocation |
...
| Session records | user_sessions table | On change |
...
A: Retention is operator-managed. Use database policies plus S3 WORM anchors (`AUDIT_ANCHOR_RETENTION_DAYS`) for compliance.
...
A: Audit events are cryptographically chained, and optional S3 Object Lock anchors preserve chain integrity.
