...
```
Format: rgwt_[role]_[random]

Example: rgwt_cicd_kX7mN2pQ9rT4wY8zA1bC3dE5fG
         ^^^^  ^^^^  ^^^^^^^^^^^^^^^^^^^^^^^^^^
         prefix role  random component
```

| Component | Description |
|-----------|-------------|
| `rgwt_` | Rack Gateway Token prefix |
| Role code | Short role identifier (viewer, ops, deployer, cicd, admin) |
...
```
Client has:  rgwt_cicd_kX7mN2pQ9rT4wY8...
...
API tokens are assigned a role that determines their permissions:
...
export RACK_GATEWAY_TOKEN=rgwt_cicd_kX7mN2pQ9rT4...

# Or use --token flag
rack-gateway apps --token rgwt_cicd_kX7mN2pQ9rT4...
```
...
```bash
curl -H "Authorization: Bearer rgwt_cicd_kX7mN2pQ9rT4..." \
  https://gateway.example.com/apps
```
  ...
  deploy:
    ...
    steps:
      ...
      - run:
          ...
          environment:
            RACK_GATEWAY_TOKEN: ${RACK_GATEWAY_TOKEN}
```
...
```bash
rack-gateway api-tokens list
```

Admins can see all tokens:

```bash
rack-gateway api-tokens list --all
```
