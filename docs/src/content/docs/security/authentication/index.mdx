...
4. After approval, code returned to CLI callback
5. CLI exchanges code + verifier for ID token
6. Session token stored in `~/.config/rack-gateway/config.json`
...
| `SESSION_TIMEOUT_MINUTES` | Idle session timeout | 5 |
