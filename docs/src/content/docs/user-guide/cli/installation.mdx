...
2. **Clone and build**
   ...
   cd rack-gateway
   go build -o rack-gateway ./cmd/rack-gateway
   ```
...
To build with WebAuthn/FIDO2 support, you need the system libraries installed (see above) and CGO enabled:

```bash
CGO_ENABLED=1 go build -o rack-gateway ./cmd/rack-gateway
```
...
```bash
CGO_ENABLED=0 go build -tags nofido -o rack-gateway ./cmd/rack-gateway
```
