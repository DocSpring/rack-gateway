...
3. Receives the authentication result via a local callback server
4. Stores the session token in your config file
...
CLI sessions are long-lived by default (configured by your gateway administrator). However, sessions can be:
...
The CLI runs a temporary local server to receive the OAuth callback. Issues can include:
- **Port conflict**: Another process is using the callback port
- **Firewall**: Local firewall blocking the callback
- **Browser security**: Some browsers block localhost redirects
...
| `GATEWAY_RACK` | Override current rack for this command |
