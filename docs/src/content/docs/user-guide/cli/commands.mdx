...
```bash
rack-gateway test-auth [flags]
```

**Flags:**
- `--mfa` - Also verify MFA status
...
rack-gateway deploy-approval status <request-id>
```
...
rack-gateway run web "convox releases rollback <release-id>" -a myapp -r production
```
