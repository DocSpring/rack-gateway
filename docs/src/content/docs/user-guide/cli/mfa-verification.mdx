...
You can verify MFA before running commands using the `--mfa` flag or `--mfa-code`:

### Using --mfa flag

```bash
# Triggers interactive MFA prompt before the command
rack-gateway deploy -a myapp --mfa
```
