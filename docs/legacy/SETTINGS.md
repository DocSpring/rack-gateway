...
# Service image patterns (JSON object mapping service name to regex pattern)
# Wildcard "*" applies to all services without a specific pattern
RGW_APP_GATEWAY_SETTING_SERVICE_IMAGE_PATTERNS='{"*": "docker.io/myapp/app:{{GIT_COMMIT}}-amd64"}'

# Override specific service while using wildcard for others
RGW_APP_MY_APP_SETTING_SERVICE_IMAGE_PATTERNS='{"*": "docker.io/myapp:{{GIT_COMMIT}}-amd64", "admin": "docker.io/myapp-admin:{{GIT_COMMIT}}"}'
